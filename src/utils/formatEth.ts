...
export const getInstance = (instance: any, method: string, param: any[] = []) => {
  const { options, getFullAccount } = useStoreMethod();
  const { Toast, bus, t } = usePublicMethod();
  return new Promise((resolve, reject) => {
    // if (!getIsCurrentChianId.value) {
...
export function equalityAddress(address: string, address2: string): boolean {
  return utils.getAddress(address) == utils.getAddress(address2);
}
export async function switchNetwork() {
  const { getProvider } = useStoreMethod();
  const signer = toRaw(getProvider.value);
  console.log('signer', signer);
  try {
    await window.ethereum.request({
      method: 'wallet_switchEthereumChain',
      params: [{ chainId: tokenObj.chainId }],
    });
